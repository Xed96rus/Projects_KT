...
def read_bme280_reg(reg, length=1):
    return i2c.readfrom_mem(BME280_ADDR, reg, length)
...
display_text(f"Pressure", row=0, col=0)  # Вывод текста на вторую строку
while True:
    try:
        ...
        if pressure_error == 1:
            lcd.clear()
            pressure_error = 0
            display_text(f"Pressure", row=0, col=0)
            display_text(f"{pressure:.3f}", row=0, col=10)  # Вывод текста на вторую
        display_text(f"{pressure:.3f}", row=0, col=10)  # Вывод текста на вторую строку
        time.sleep(1)
    except Exception as error:
        print(' VRUBI')
        display_text(f"Net datchika", row=1, col=0)  # Вывод текста на вторую строку
        display_text(f"Old_Pressure {pressure:.1f}", row=0, col=0)  # Вывод текста на первую стр
        time.sleep(1)
        pressure_error = 1
    ...
    try:
        if disp_error == 1:
            ...
            disp_error = 0
            try:
                display_text(f"{pressure:.3f}", row=0, col=10)
                display_text(f"{pressure:.3f}", row=0, col=10)
            except Exception as error:
                pass
